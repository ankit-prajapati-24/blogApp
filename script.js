document.addEventListener('DOMContentLoaded', () => {
  ...
  function showMessage(message, type) {
    ...
    messageBox.className = "fixed top-6 right-6 z-50 px-4 py-2 rounded-lg font-medium transition-all";
    messageBox.classList.add(type === 'success' ? 'bg-green-700 text-green-200' : 'bg-red-700 text-red-200', 'show');
  ...
  function createBlogCard(blog) {
    ...
    return `
      <div class="bg-gray-800 h-[400px] w-[400px] rounded-xl shadow-lg overflow-hidden transform transition hover:scale-105">
        <div class="p-6 text-gray-200">
          <h2 class="text-xl font-bold mb-2 text-cyan-400">${blog?.title || 'Untitled'}</h2>
          <h5 class="text-xs font-bold mb-2 text-gray-400">ID: ${blog?._id || 'Untitled'}</h5>
          <p class="text-sm text-gray-400 mb-4">By ${blog?.author || 'Unknown'} on ${date}</p>
          <p class="text-gray-300 mb-6">${snippet || ''}</p>
          <div class="flex flex-col sm:flex-row sm:space-x-3 space-y-2 sm:space-y-0">
            <button data-id="${blog?._id}" data-action="edit" class="flex-1 bg-yellow-500 hover:bg-yellow-600 text-black px-4 py-2 rounded-lg">Edit</button>
            <button data-id="${blog?._id}" data-action="delete" class="flex-1 bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg">Delete</button>
          </div>
        </div>
      </div>
    `;
  ...
  function renderBlogs(blogs) {
    blogsContainer.innerHTML = blogs?.length
      ...
      : '<p class="text-center text-gray-400">No blogs yet.</p>';
  }
  ...
  blogsContainer.addEventListener('click', async (e) => {
    ...
    if (action === 'edit') {
      ...
      try {
        ...
        const result = await res.json();
        if (res.ok && result?.blog) setupForm(result.blog);
        else showMessage('Failed to fetch blog', 'error');
      } catch {
